...
send_url <- 'https://api.github.com/repos/phuse-org/phuse-scripts/contents/data/send';
...
server <- function(input, output,session) {
  ...
  output$selectGitHubStudy <- renderUI({
    # Req <- GET(paste('https://api.github.com/repos/phuse-org/phuse-scripts/contents/data/send'),authenticate(userGitHub,passwordGitHub))
	# Req <- GET(send_url,authenticate(userGitHub,passwordGitHub))
	Req <- GET(send_url)
  ...
  loadData <- reactive({
    ...
    if (input$dataSource=='GitHub') {
	  ...
	  values$path <- paste0('https://github.com/phuse-org/phuse-scripts/tree/master/data/send/', input$selectGitHubStudy)
    }
    path <- values$path
    if (input$dataSource=='local') {
      ...
      # Dataset <- load.GitHub.xpt.files(studyDir=StudyDir,authenticate=TRUE,User=userGitHub,Password=passwordGitHub)
