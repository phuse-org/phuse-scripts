                ...
                <div id="menu">
                    <ul>
                        ...
                        <xsl:if  test="/odm:ODM/odm:Study/odm:MetaDataVersion/def:ValueListDef">
                            ...
                            <!-- Parent Child Start -->
		            <xsl:if  test="not(starts-with(@OID,'ValueList.PP.PPCAT')) and not(starts-with(@OID,'ValueList.CL.CLCAT')) and not(starts-with(@OID,'ValueList.PC.PCCAT')) and not(starts-with(@OID,'ValueList.LB.LBCAT'))">
<li class="toc2" >
     <xsl:attribute name="id">
     ...
     <a class="tocItem">
		...
		#<xsl:value-of select="@OID"/>
			   ...
			   <xsl:if  test="starts-with(@OID,'ValueList.PP.PPCAT') or starts-with(@OID,'ValueList.CL.CLCAT') or starts-with(@OID,'ValueList.PC.PCCAT') or starts-with(@OID,'ValueList.LB.LBCAT')">
		  <xsl:if  test="not(starts-with(@OID,'ValueList.PP.PPCAT.')) and not(starts-with(@OID,'ValueList.CL.CLCAT.')) and not(starts-with(@OID,'ValueList.PC.PCCAT.')) and not(starts-with(@OID,'ValueList.LB.LBCAT.'))">
            ...
            <xsl:for-each select="/odm:ODM/odm:Study/odm:MetaDataVersion/def:ValueListDef">
                ...
                <xsl:for-each select="./odm:ItemRef">
                    ...
                    <tr>
                        <td>
                        <!-- Parent Child Start -->
                           <xsl:if  test="not(starts-with($valueDefOid,'PP.PPCAT.')) and not(starts-with($valueDefOid,'CL.CLCAT.')) and not(starts-with($valueDefOid,'PC.PCCAT.')) and not(starts-with($valueDefOid,'LB.LBCAT.'))">
		          <a>
                  <xsl:attribute name="Name">
...
<xsl:if  test="starts-with($valueDefOid,'PC.PCCAT')">
				<xsl:choose>
				<xsl:when test="$valueDef/def:ValueListRef/@ValueListOID!=''">
					PCCAT
				</xsl:when>
