...
%macro params;
	...
	%if %upcase(&run_location.) = LOCAL %then %do; 
		...
		%let studypath = C:\Documents and Settings\MATTOK\Desktop\Data\NDA201532;
		...
		%let templatepath = C:\Documents and Settings\MATTOK\Desktop\SL_Templates\DemographicsGeneral;
		%let template = Demographics_Template.xls; 

		/* location and filename of the output */
		%let outpath = C:\Documents and Settings\MATTOK\Desktop\Output;
...
%macro dm_setup;
	...
	%if &dm_subj_gt0. and &setup_req_var. %then %do;
		...
		proc sql noprint;
			...
			from dm	
			where armcd not in ('SCRNFAIL','NOTASSGN')
		...
		data dm;
			...
			%if &dm_armcd. %then %do;
				if armcd in ('SCRNFAIL','NOTASSGN') then delete;
			%end;
