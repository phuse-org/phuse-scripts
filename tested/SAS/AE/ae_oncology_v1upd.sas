...
options minoperator;
options missing='' mlogic symbolgen;
...
%macro params(in_panel_title=AE Toxicity,
			in_panel_desc=,
			in_saspath=,
			in_utilpath=,
			in_outpath=,
			in_oncaeout=AE Toxicity Analysis.xls,
			in_errout=AE Toxicity Error Summary.xls,
			in_studypath=,
			in_ndabla=,
			in_studyid=DeID,
			in_meddrapath=,
			...
			in_ctl=2,
			in_cmpgr=all,
			in_cmptrm = %str(soc_name,pt_name),
			in_cmpsort = rr,
			in_cc=0.5);
	...
	%if %upcase(&run_location.) = LOCAL %then %do;
		...
		%let panel_title = &in_panel_title;
		...
		%let oncaeout = &outpath.\&in_oncaeout.;
		%let errout = &outpath.\&in_errout.;
		...
		%let toxgr_grp5_sw = &in_toxgr_grp5_sw;
