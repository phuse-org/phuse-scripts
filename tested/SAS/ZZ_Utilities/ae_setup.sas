...
2015-08-13  DC  Program now uses only Preferred term to make the MedDRA hierarchy (line 465)
...
%macro setup(mdhier=N,dme=N);
	...
	proc sql noprint;
		...
		select distinct usubjid,  /* Added "distinct" 11/13/2015 by PG. Removes duplicate records from dataset all_ex if variable exstdtc is removed from ex for analysis. */
...
/*				     max(input(substr(exendtc,1,10),? e8601da.))*/
	...
	%if &mdhier. = Y %then %do;
		...
		/* The below data step was added 11/20/2015 by Peter Glass as part of effort to use only AEDECOD to look up MedDRA terms (line 465) */;

		data meddra_primary_soc; 
			set meddra.mdhier_%sysfunc(translate(&ver.,'_','.'));
			if primary_soc_fg = 'Y';
		run;
		...
		data ds_base_meddra(drop=aebodsys aedecod)
			...
			if _n_ = 1 then do;
				declare hash h(dataset:"meddra_primary_soc");
