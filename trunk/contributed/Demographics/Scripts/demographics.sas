...
%macro dm_setup;
	...
	%if &dm_subj_gt0. and &setup_req_var. %then %do;
		...
		data lkp_age;
			...
			else age_flag = trim(put(min_age,fract8. -l))||' '||trim(min_ageu_txt)||' <= Age <'
	                        ||trim(put(max_age,fract8. -l))||' '||trim(max_ageu_txt);
